...
public class AES {
    ...
    public static byte[] generateKey() {
        try {
            // Key size can be adjusted based on your encryption algorithm requirements (e.g., 128, 192, or 256 bits)
    ...
    public static byte[] generateIV() {
        try {
            // IV size should match the block size of the encryption algorithm (e.g., 16 bytes for AES)
    ...
    public static String byteArrayToHexString(byte[] array) {
        ...
        for (byte element : array) {
            ...
            hexStringBuilder.append(hexElement.length() == 1 ? "0" + hexElement : hexElement);
        }
    ...
    public static byte[] hexStringToByteArray(String hexString) {
        ...
        for (int i = 0; i < length; i += 2) {
            String hexElement = hexString.substring(i, i + 2);
            newArray[i / 2] = (byte) Integer.parseInt(hexElement, 16);
        }

        return newArray;
    }

    public static byte[] trimZeroesFromEnd(byte[] byteArray) {
        // Find the index of the last non-zero byte
        int lastNonZeroIndex = byteArray.length - 1;
        while (lastNonZeroIndex >= 0 && byteArray[lastNonZeroIndex] == 0) {
            lastNonZeroIndex--;
        }

        // If the array contains only zero bytes, return an empty array
        if (lastNonZeroIndex < 0) {
            return new byte[0];
        }

        // Create a new array with non-zero bytes
        byte[] newByteArray = Arrays.copyOfRange(byteArray, 0, lastNonZeroIndex + 1);
